...
% 	plotroc(y,x,model);
	...
	elseif exist('params') && ~ischar(params)
		model = params;
		[predict_label,mse,deci] = predict(y,x,model); % the procedure for predicting
...
function [deci,label_y] = get_cv_deci(prob_y,prob_x,param,nr_fold)
	...
	for i=1:nr_fold % Cross training : folding
		...
		train_ind = [1:l]';
		train_ind(test_ind) = [];
		model = train(prob_y(train_ind),prob_x(train_ind,:),param);	   
		[predict_label,mse,subdeci] = predict(prob_y(test_ind),prob_x(test_ind,:),model);
