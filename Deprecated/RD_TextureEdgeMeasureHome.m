...
for v = 1: numFrames;
EntireVideo(:,:,v) = imresize(rgb2gray(read(vidObj,1)),RESIZEVALUE);
end
    ...
    for i = 1 : saveNumber
        ...
        for f = 0 : WINDOWSIZE - 1
            ...
            if isempty(frameBufferTexture{s + f,1})
                ...
                %% Save the GLCM Matrix so we don't have to recompute it
                % if ~GLCM_Exists
                %     GLCM_Save(GLCMData, FrameNumber, BASEOFFSET, SYMMETRY, LEVELS,...
                %         RESIZEVALUE,RANGE, PYR,CurrentPyramidStructure,...
                %         DATA_VIDEO_CHOSENSET,VIDEOLISTITEM);
                % end
