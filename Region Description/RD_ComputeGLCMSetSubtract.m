...
for G = 1 : M
    ...
    GLCMFinalPartitionSet = cell(T,prod(CurrentGrid));
    for F = 1 : T - 1
        for p = 1: prod(CurrentGrid)
        Difference = ... 
        GLCMPartitionSet{F + 1,p} - GLCMPartitionSet{F,p} ;
        Difference(Difference < 0) = 0;
        GLCMFinalPartitionSet{F,p}= Difference;
        end
    end
    
    OUTPUT{G} = GLCMFinalPartitionSet;
end
