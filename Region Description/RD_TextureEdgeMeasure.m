...
for q = 1: PYRSIZE(1)
    ...
    for i = 1 : saveNumber
        ...
        CORRELATIONSeries = zeros(WINDOWSIZE,prod(CurrentPyramidStructure));
        for f = 0 : WINDOWSIZE - 1
            ...
            if isempty(frameBufferTexture{s + f,1})
                ...
                GLCMPartitionCollection = GLCME_ComputeGLCM( I, CurrentPyramidStructure, OFFSETS,LEVELS);
                ...
                for p = 1: prod(CurrentPyramidStructure)
                    ...
                    Y = rem(p-1,CurrentPyramidStructure(1))+1;
                    X = (p-Y)/CurrentPyramidStructure(1) + 1;
                    
                    GLCMData{p} = sum(cat(3,GLCMPartitionCollection{Y,X,:}),3);
                    statss = graycoprops(GLCMData{p},{'Contrast','Energy','Correlation'});
                    ENERGYSeries(f + 1,p) =  statss.Energy;
                    CONTRASTSeries(f + 1,p)  =  statss.Contrast;
                    CORRELATIONSeries(f + 1,p)  =  statss.Correlation;
                ...
                for p = 1: prod(PYR(q,:))
                    ...
                    ENERGYSeries(f + 1,p) =  frameBufferTexture{s + f,p}.Energy;
                    CONTRASTSeries(f + 1,p)  =  frameBufferTexture{s + f,p}.Contrast;
                    CORRELATIONSeries(f + 1,p)  = frameBufferTexture{s + f,p}.Correlation;
                end
            end
            
            
            %% REWRITE THE FOLLWING SO THAT AFTER EDGE EACH EDGE LIST IS CREATED, MAKE IT@S OWN FUNCTION
            ...
            if isempty(frameBufferEdge{s + f,1})
                ...
                for p = 1: prod(CurrentPyramidStructure)
                    ...
                    [ XRange,YRange ] = MISC_SplitMatInd( BW,CurrentPyramidStructure(1),...
                        CurrentPyramidStructure(2),Y,X );
                    
                    BWSub = BW(YRange,XRange);
                    EDGESseries(f + 1,p) = sum(sum(BWSub)) / numel(BWSub);
                    
                end
                frameBufferEdge{s + f,1} = BW;
            else
                BW =  frameBufferEdge{s + f,1};
        ...
        for p = 1: prod(PYR(q,:))
            ...
            subSample(p,9) = mean(CORRELATIONSeries(:,p));
            subSample(p,10) = var(CORRELATIONSeries(:,p));
        end
