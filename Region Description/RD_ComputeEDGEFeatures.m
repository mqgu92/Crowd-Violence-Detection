function [ FinalFeatureSet ] = RD_ComputeEDGEFeatures( EDGE_SET,WINDOW_SIZE,WINDOW_SPLIT,WINDOW_SKIP )
% Using a GLCM set compute the features appropraitely
%

%Features = 'Energy,Contrast,Correlation,Homogeneity'

[M, ~] = size(EDGE_SET); % Number of different GLCM partitions
[N, ~] = size(EDGE_SET{1});  % Number of frames
F = 1;
...
for P = 1 : M
    ...
    Partition = EDGE_SET{P};
    [~, PartitionSize] = size(Partition);
    
    SubFeatureSet = EDGE_SET{P};
